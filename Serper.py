...
def get_top_search_results(query:str):
    """
    Fetches top search results using the Serper API.
    
    Parameters:
    - query (str): The search term

    Returns:
    - List of dicts containing title, link, and snippet for each result
    """
    url = "https://google.serper.dev/search"
    headers = {
        "X-API-KEY": '6d5f6ce7b2818f73fc3c32bbab41a06346f3a771',
...
"""
    ...
    webpage_extractor = AssistantAgent(
    name="WebPageExtractor",
    model_client=model_client,
    description="Extracts and summarizes main content from web pages given a URL.",
    system_message="""
    You are a web content extraction expert. When given a URL, use the extract_page_text tool to fetch and summarize the main content of the page. Return only the main text, no ads or navigation.
    Format your output as:
    - URL: [the url]
    - Extracted Content: [main text, up to 3000 characters]
    """,
...
2. For each query, use the **get_top_search_results** tool to get the latest web results (titles, snippets, URLs).
...
""",
    ...
    team = SelectorGroupChat(
        participants=[
            user_proxy, supervisor, idea_enhancer, market_questionnaire,
            estimator_agent, ba_agent, market_agent, swot_agent,
            gtm_agent, build_vs_outsource_agent, webpage_extractor
        ],
        model_client=model_client,
