...
3. For the most relevant URLs, use the **extract_page_text** tool to extract and summarize the main content of the page (ignore ads, navigation, etc.). Strictly use the extract_page_text on the URL's provided by the google_search tool only. 
...
3. regulations for commercial drone delivery India 2025
...
"""
    ...
    # tech_stack_engine = AssistantAgent(name = "tech_stack_engine",
    #     model_client=model_client, description=
    #     """Based on the Market research provides the tech stack for the particular development.""",
    #     system_message=f"""You are the **Tech Stack Engine**.

    # Your purpose is to take any user-provided idea or project description and generate a **recommended technology stack** tailored to that idea.  

    # ### Responsibilities:
    # 1. **Analyze the Idea**  
    # - Break the project into key technical components (Frontend, Backend, Database, Infrastructure, Integrations, DevOps, Tools).  

    # 2. **Recommend the Stack**  
    # - Suggest the most suitable technologies, frameworks, and services for each component.  
    # - Provide one **primary recommendation** and (if useful) one **alternative** per layer.  

    # 3. **Justify Choices**  
    # - Briefly explain why each technology is a good fit for the given idea.  

    # 4. **Format Output**  
    # - Always present results in a **clear, layered format** like this:  

    # ---

    # ### 🛠 Recommended Tech Stack for: [User’s Idea]  

    # **Frontend**  
    # - Framework: React (Next.js) → *SEO-friendly, scalable, good for SPAs*  
    # - Styling: Tailwind CSS → *modern UI, fast iteration*  

    # **Backend**  
    # - Language: TypeScript  
    # - Framework: Node.js (NestJS) → *modular, easy to scale*  

    # **Database**  
    # - Primary: PostgreSQL → *structured, reliable*  
    # - Cache: Redis → *fast query response*  
    ...
    # **Integrations**  
    # - Payments: Stripe → *secure, global support*  
    ...
    # - Keep recommendations **practical and widely adopted**.  
    # - Adapt suggestions based on whether the project is a **MVP** or an **enterprise-scale solution**.  
    ...
    supervisor = AssistantAgent(
        ...
        system_message="""### Instruction###
         ...
         4. BusinessAnalyst      -  After Market research, it generates the documents like (BRD, SRS, FRD, SOW, or RFP)
         5. GTM_Strategy_Generato-  After Business documents are finalised by the BusinessAnalyst, It provides the go to market strategy.
    ...
    team = SelectorGroupChat(
        participants=[user_proxy,supervisor, idea_enhancer,market_questionnaire, estimator_agent, ba_agent, market_agent, gtm_agent],
        selector_prompt= '''Select the next agent to progress a 6-phase workflow in this strict order:
1) IdeaEnhancer → 2) MarketQuestionnaire → 3) MarketResearcher → 4) BusinessAnalyst → 5) GTM_Strategy_Generator → 6) estimator_agent.
Only select User when an agent explicitly requests clarification.
''',
        model_client=model_client,
